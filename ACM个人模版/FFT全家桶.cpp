namespace FFT {
  ...
  void fft(std::vector<Complex> &a, int n = -1) {
    if (n == -1) {
      n = a.size();
    }
    assert((n & (n - 1)) == 0);
  ...
  std::vector<T> multiply(const std::vector<T> &a, const std::vector<T> &b) {
    ...
    fft(fa, sz);
    std::vector<T> res(need);
    for (int i = 0; i < need; i++) {
      res[i] = fa[i].r + 0.5; // watch out that fa[i].r < 0
    }
    return res;
  }
  std::vector<int> multiply_mod(const std::vector<int> &a, const std::vector<int> &b, int m, int eq = 0) {
    int need = a.size() + b.size() - 1;
    int nbase = need > 1 ? 32 - __builtin_clz(need - 1) : 0;
    ensure_base(nbase);
    int sz = 1 << nbase;
  ...
  std::vector<int> square_mod(const std::vector<int> &a, int m) {
